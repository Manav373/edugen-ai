...
import { FileText, Download, Loader2, Copy, FileDown, BookmarkPlus, BookmarkCheck, Volume2, VolumeX, Sun, Moon, History, Sparkles, Maximize2, X, Brain, Check, GraduationCap, ArrowUp } from 'lucide-react';
...
const AssignmentForm: React.FC = () => {
    ...
    const handleDownloadPDF = useReactToPrint({
        ...
        documentTitle: `Assignment_${subject || 'General'}_Answers`,
    });
    ...
    const handleDownloadWord = () => {
        ...
        <head>
            <meta charset="utf-8">
            <title>Assignment Answers</title>
            <style>
                body { 
        ...
        link.href = url;
        link.download = `Assignment_${subject || 'General'}_${Date.now()}.doc`;
    ...
    const saveAnswer = (bookmark: boolean = false) => {
        ...
        localStorage.setItem('assignmentAnswers', JSON.stringify(updated));
    };
    ...
    const handleGenerate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.answer) {
                ...
                setTimeout(() => saveAnswer(false), 1000); // Auto-save
            }
        } catch (error) {
                    ...
                    >
                        ...
                        >
                            ...
                            <textarea
                                ...
                                className="flex-1 w-full bg-gray-950 p-4 sm:p-6 text-base sm:text-lg text-gray-200 outline-none resize-none font-mono"
            ...
            <div className={`transition-all duration-500 ${result ? 'w-full lg:w-1/2' : 'w-full max-w-4xl mx-auto'} h-auto lg:h-full lg:overflow-y-auto custom-scrollbar pr-2`}>
                <div className="flex items-center justify-between mb-4 sm:mb-6">
                    <h2 className="text-2xl sm:text-3xl font-bold flex items-center gap-2 sm:gap-3 text-white">
                        ...
                        <span className="hidden sm:inline">Assignment Solver</span>
                                    ...
                                    <div className="space-y-2">
                                        {savedAnswers.map((saved) => (
                                            <div
                                                ...
                                                className="p-3 bg-gray-800/30 hover:bg-gray-800 rounded-lg cursor-pointer border border-gray-700/50 hover:border-blue-500/50 transition-all flex justify-between items-center group"
                                            >
                                                <div className="truncate flex-1 pr-4">
                                                    <div className="flex items-center gap-2">
                                                        <span className="font-medium text-blue-300">{saved.subject || 'General'}</span>
                    ...
                    <form onSubmit={handleGenerate} className="space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <button
                                ...
                                className={`w-full py-4 border-2 border-dashed rounded-xl transition-all group min-h-[80px] ${processingFiles ? 'border-blue-500 bg-blue-500/5' : 'border-gray-700 hover:border-blue-500 hover:bg-blue-500/5'
                                    }`}
                            >
                                <div className="flex flex-col items-center gap-2">
                                    ...
                                    <span className="text-gray-400 group-hover:text-blue-400 font-medium">
                                        {processingFiles ? 'Processing Files...' : 'Upload Assignment (PDF / Images)'}
                                    </span>
                                </div>
                            </button>
                            {files.length > 0 && (
                        ...
                        <div className="relative">
                            ...
                            <div className="relative">
                                ...
                                <button
                                    ...
                                    className="absolute top-2 right-2 p-2 bg-gray-800/80 hover:bg-blue-600 rounded-lg text-gray-300 hover:text-white transition-all backdrop-blur-sm"
                                    title="Expand Editor"
                                >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full p-3 bg-gray-900/50 border border-gray-600 rounded-xl text-white focus:border-blue-500 outline-none transition-all"
                            ...
                            {/* Answer Style */}
                            <div>
                                <label className="text-sm font-semibold text-gray-300 mb-2 block">Answer Style</label>
                                <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
                                    {[
                                        { id: 'academic', label: 'Academic', icon: 'aaa' },
                                        { id: 'simple', label: 'Simple', icon: 'bbb' },
                                        { id: 'bullet_points', label: 'Bullet Points', icon: 'ccc' },
                                    ].map((s) => (
                                        <button
                                            key={s.id}
                                            type="button"
                                            onClick={() => setStyle(s.id)}
                                            className={`py-2 px-1 sm:px-3 rounded-lg text-xs sm:text-sm font-medium transition-all border min-h-[44px] flex items-center justify-center ${style === s.id
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-5 gap-1.5 sm:gap-2">
                                {['1', '2', '3', '4', '5'].map((m) => (
                                    <button
                                        ...
                                        type="button"
                                        onClick={() => setMarks(m)}
                                        className={`py-2 sm:py-3 rounded-xl font-bold transition-all min-h-[44px] text-sm sm:text-base ${marks === m
                            ...
                            <p className="text-xs text-center text-gray-500 mt-2">
                                {marks === '1' && 'One-liners & direct answers'}
                                {marks === '3' && 'Balanced paragraphs & examples'}
                                {marks === '5' && 'Comprehensive essay-style explanations'}
                            </p>
                        </div>
                        ...
                        <button
                            ...
                            className="w-full py-3 sm:py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold text-base sm:text-lg rounded-xl hover:shadow-lg hover:shadow-blue-500/20 hover:scale-[1.01] transition-all disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-2 min-h-[44px]"
                        >
                            {isGenerating ? (
                                <>
                    ...
                    >
                        <div className="bg-gray-800/80 backdrop-blur-md border border-gray-700 rounded-t-2xl p-4 flex items-center justify-between shadow-lg z-10">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={handleTextToSpeech} className="p-2 hover:bg-gray-700/50 rounded-lg text-gray-400 hover:text-blue-400 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center">
                        ...
                        >
                            <div className="prose max-w-none print:prose-neutral prose-invert">
                                ...
                                <div className="hidden print:block mb-6 pb-6 border-b border-gray-200">
                                    <h1 className="text-2xl font-bold text-gray-900">Assignment Solution</h1>
